...
namespace Drupal\alotcare\Plugin\rest\resource;
...
/**
 * Represents StaffAvailability records as resources.
 *
 * @RestResource (
 *   id = "alotcare_staff_availability",
 *   label = @Translation("StaffAvailability"),
 *   uri_paths = {
 *     "canonical" = "/api/alotcare-staff-availability/{id}",
 *     "https://www.drupal.org/link-relations/create" = "/api/alotcare-staff-availability"
 *   }
 * )
 *
 * @DCG
 ...
 */
class StaffAvailabilityResource extends ResourceBase implements DependentPluginInterface {
  ...
  public function post($data) {
    ...
    $id = $this->dbConnection->insert('alotcare_staff_availability')
      ->fields($data)
      ->execute();

    $this->logger->notice('New staffavailability record has been created.');
  ...
  public function delete($id) {
    ...
    $this->dbConnection->delete('alotcare_staff_availability')
      ->condition('id', $id)
      ->execute();

    $this->logger->notice('StaffAvailability record @id has been deleted.', ['@id' => $id]);
  ...
  protected function loadRecord($id) {
    $record = $this->dbConnection->query('SELECT * FROM {alotcare_staff_availability} WHERE id = :id', [':id' => $id])->fetchAssoc();
    if (!$record) {
  ...
  protected function updateRecord($id, array $record) {
    ...
    $this->dbConnection->update('alotcare_staff_availability')
      ->fields($record)
      ->condition('id', $id)
      ->execute();

    $this->logger->notice('StaffAvailability record @id has been updated.', ['@id' => $id]);
